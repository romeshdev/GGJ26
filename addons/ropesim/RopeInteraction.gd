...
## (Optional) If set, the target node's movement will be constrained to this path.
@export var constraint_path: Path2D
...
func _on_after_update() -> void:
	...
	if constraint_path:
		# Convert the global rope position to the path's local space
		var local_pos = constraint_path.to_local(target_pos)
		# Get the nearest point on the curve
		var snapped_local_pos = constraint_path.curve.get_closest_point(local_pos)
		# Convert back to global space
		target_pos = constraint_path.to_global(snapped_local_pos)
	# --- Constraint Logic End ---

	var diff := target_pos - target_node.global_position
	var distance = target_node.global_position.distance_to(target_pos)
	...
	match position_update_mode:
		...
		PositionUpdateMode.MoveAndSlide:
			...
			# Note: For Path2D constraints, move_and_slide might fight the path 
			# if collisions are present. Usually SetGlobalPosition or EmitSignal 
			# is preferred for strictly constrained movement.
			...
			if distance > 1:
				rope.rope_length += 1
				rope.num_segments = rope.rope_length / 10
				rope.max_endpoint_distance += 1
